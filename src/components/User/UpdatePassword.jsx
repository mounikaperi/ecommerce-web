...
import { UPDATE_PASSWORD } from '../../constants/userConstants';
...
const UpdatePassword = () => {
  ...
  useEffect(() => {
    ...
    if (isUpdated) {
      ...
      dispatch({ type: UPDATE_PASSWORD.UPDATE_PASSWORD_RESET });
    }
            ...
            <form onSubmit={updatePasswordSubmitHandler} className='p-5 sm:p-14'>
              <div className = 'flex flex-col gap-4 items-start'>
                <div className='flex flex-col w-full justify-between sm:flex-col gap-3 items-center'>
                  <TextField fullWidth label='Current Password' type='password' name='oldPassword' value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} required />
                  <TextField fullWidth label='New Password' type='password' name='newPassword' value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required />
                  <TextField fullWidth label='Confirm Password' type='password' name='confirmPassword' value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
                </div>
