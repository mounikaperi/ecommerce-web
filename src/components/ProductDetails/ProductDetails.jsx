...
import { NEW_REVIEW } from '../../constants/productConstants';
...
const ProductDetails = () => {
  ...
  useEffect(() => {
    ...
    if (success) {
      ...
      dispatch({ type: NEW_REVIEW.NEW_REVIEW_RESET });
    }
              ...
              <div className='w-full sm:w-2/5 sm:sticky top-16 sm:h-screen'>
                <div className='flex flex-col gap-3 m-3'>
                  <div className='w-full h-full pb-6 border relative'>
                    <Slider {...settings}>
                      {product.images && product.images.map((item, i) => (
                        <img draggable="false" className='w-full h-96 object-contain' src={item.url}
                          alt={product.name} key={i} />
                      ))}
                    </Slider>
                    <div className='absolute top-4 right-4 shadow-lg bg-white w-9 h-9 border flex items-center
                  ...
                  <div className='w-full flex gap-3'>
                    { product.stock > 0 && (
                      <button onClick={itemInCart ? goToCart : addToCartHandler} className='p-4 w-1/2 flex
                      items-center justify-center gap-2 text-white bg-primary-yellow rounded-sm shadow hover:shadow-lg'>
                        <ShoppingCartIcon />
                        { itemInCart ? "GO TO CART" : "ADD TO CART"}
                      </button>
                    )}
                    <button onClick={buyNow} disabled={product.stock < 1} className={product.stock < 1 ? "p-4 w-full flex items-center justify-center gap-2 text-white bg-red-600 cursor-not-allowed rounded-sm shadow hover:shadow-lg" : "p-4 w-1/2 flex items-center justify-center gap-2 text-white bg-primary-orange rounded-sm shadow hover:shadow-lg"}>
                      <FlashOnIcon />
              ...
              <div className='flex-1 py-2 px-3'>
                <div className='flex flex-col gap-2 mb-4'>
                  ...
                  <div className='flex gap-8 mt-2 items-center text-sm'>
                    <img draggable="false" className='w-20 h-8 p-0.5 border object-contain' src={product.brand?.logo.url}
                  ...
                  <div className='w-full mt-4 rounded-sm border flex flex-col'>
                    <div className='flex justify-between items-center border-b px-6 py-4'>
                      ...
                      <button onClick={handleDialogClose} className='shadow bg-primary-yellow text-white px-4 py-2
                        rounded-sm hover:shadow-lg'>Rate Product</button>
                    </div>
                    <Dialog aria-labelledby='review-dialog' open={open} onClose={handleDialogClose}>
                      ...
                      <DialogContent className='flex flex-col m-1 gap-4'>
                        ...
                        <TextField label='Review' multiline rows={3} sx={{width:400}} size="small" variant="outlined" value={comment} onChange={(e) => setComment(e.target.value)} />
                      </DialogContent>
                      <DialogActions>
