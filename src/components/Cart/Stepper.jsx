...
const Step = ({ isDesc, label, desc, index }) => {
  return (
    <div className="flex bg-white shadow px-4 py-3 pb-4 rounded-sm">
      <span className="mt-2 ml-2 mr-4 h-5 w-5 flex items-center justify-center text-xs font-medium bg-gray-100
        rounded-sm text-primary-blue">{index + 1}</span>
      <div className="flex flex-col mt-1 gap-0.5">
        <h2 className="font-medium text-gray-500 flex items-center gap-2">{label}
          { isDesc && (<span className="text-primary-blue mb-1"><CheckIcon sx={{ fontSize: "20px" }} /></span>)}
        </h2>
        {isDesc && desc}
      </div>
    </div>
